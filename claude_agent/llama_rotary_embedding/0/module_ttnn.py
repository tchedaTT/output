...
class LlamaRotaryEmbedding:
    """
    Rotary Position Embedding for LLaMA model ported to ttnn
    """
    ...
    def __init__(self, config, torch_params=None, device=None):
        ...
        # Initialize inv_freq and attention_scaling
    ...
    def rotate_half(x):
        ...
        x1 = ttnn.slice(x, [0] * (len(x.shape) - 1) + [0], list(x.shape[:-1]) + [half_dim])
        x2 = ttnn.slice(x, [0] * (len(x.shape) - 1) + [half_dim], list(x.shape))
    ...
    def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
        ...
        """
        cos = ttnn.unsqueeze(cos, unsqueeze_dim)
        sin = ttnn.unsqueeze(sin, unsqueeze_dim)

        # Apply rotary embedding: (q * cos) + (rotate_half(q) * sin)
        ...
        # Apply rotary embedding: (k * cos) + (rotate_half(k) * sin)
    ...
    def __call__(self, x, position_ids):
        """
        Forward pass - expects x and position_ids to already be ttnn tensors on device
        """
        # Expand inv_freq to match position_ids batch size
        batch_size = position_ids.shape[0]
        inv_freq_shape = [batch_size] + [1] * (len(self.inv_freq.shape) - 1) + [1]
        inv_freq_expanded = ttnn.unsqueeze(ttnn.unsqueeze(self.inv_freq, 0), -1)
        inv_freq_expanded = ttnn.repeat(inv_freq_expanded, [batch_size, 1, 1])

        # Expand position_ids
        position_ids_expanded = ttnn.unsqueeze(position_ids, 1)
        ...
        cos = ttnn.multiply(cos, self.attention_scaling, memory_config=ttnn.DRAM_MEMORY_CONFIG)
        sin = ttnn.multiply(sin, self.attention_scaling, memory_config=ttnn.DRAM_MEMORY_CONFIG)
