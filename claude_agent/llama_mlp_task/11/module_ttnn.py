...
class LlamaMLP:
    ...
    def __init__(self, config, torch_params=None, device=None):
        ...
        if torch_params is not None:
            ...
            # Extract weights for the three linear layers
            ...
            # Note: ttnn.linear expects (B,C) weights while torch uses (C,B)
    ...
    def __call__(self, x):
        ...
        gate_silu = ttnn.multiply(gate_out, gate_sigmoid, memory_config=ttnn.DRAM_MEMORY_CONFIG)

        # Element-wise multiply gate and up outputs
        gated = ttnn.multiply(gate_silu, up_out, memory_config=ttnn.DRAM_MEMORY_CONFIG)

        # Down projection
        down_out = ttnn.linear(gated, self.down_weight, bias=self.down_bias, memory_config=ttnn.DRAM_MEMORY_CONFIG)
